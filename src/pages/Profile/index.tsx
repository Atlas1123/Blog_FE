...
const ProfilePage = () => {
    ...
    return (
        ...
        <Button type="button" onClick={connectMyWallet}>Connect my wallet</Button>
        <Text className="public-key" mr={"1"}>{etherBalance && parseFloat(formatEther(etherBalance)).toFixed(3)} ETH</Text>
        {account ? (
            <>
            <Text className="total-eth">{account &&
                        `${account}`}</Text>
            </>
        ): (<></>) }
        </>
